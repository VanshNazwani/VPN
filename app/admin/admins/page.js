...
//     if (!newAdminData.fullName || !newAdminData.email) {
//       toast.error('Please fill in all fields');
//       return;
//     }
...
//         toast.success('Login credentials have been sent to the admin email.');
//         setIsDialogOpen(false);
//         setNewAdminData({ fullName: '', email: '' });
//         fetchAdmins();
//       } else {
//         toast.error(data.message || 'Failed to create administrator');
//       }
...
import { useConfirmDialog } from '../../../components/custom-confirm-dialog';
...
export default function AdminManagePage() {
  ...
  const toggleAdminStatus = async (adminId, currentStatus) => {
    ...
    const confirmed = await confirmDialog(
      `Are you sure you want to ${action} this administrator? ${currentStatus ? 'They will no longer be able to access the admin panel.' : 'They will regain access to the admin panel.'}`,
      `${action.charAt(0).toUpperCase() + action.slice(1)} Administrator`
    );
  ...
  const deleteAdmin = async (adminId, adminName) => {
    const confirmed = await confirmDialog(
      `Are you sure you want to permanently delete "${adminName}"? This action cannot be undone and will remove their access to the admin panel immediately.`,
